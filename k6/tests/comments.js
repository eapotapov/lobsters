...
function hitComments(base) {
  const res = http.get(`${base}/comments`);
  check(res, { "status 200": (r) => r.status === 200 });
  sleep(1);
}

export function mariadb() {
  hitComments(VERSIONS.mariadb);
}

export function sqliteBroken() {
  hitComments(VERSIONS["sqlite-broken"]);
}

export function sqliteFixed() {
  hitComments(VERSIONS["sqlite-fixed"]);
}
